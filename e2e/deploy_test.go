...
func cacheImages(t *testing.T, clientset *kubernetes.Clientset) {
	...
	case <-time.After(20 * time.Second):
		...
		t.Errorf("Timeout waiting a ready daemonset")
	}
	...
	for _, pod := range pods.Items {
		if pod.ObjectMeta.OwnerReferences[0].Name == daemonsetName {
			for _, containerStatus := range pod.Status.ContainerStatuses {
				if !*containerStatus.Started && containerStatus.State.Waiting != nil {
					t.Errorf("Waiting container '%v'. Reason: '%v' message: '%v'", containerStatus.Name, containerStatus.State.Waiting.Reason, containerStatus.State.Waiting.Message)
				}
			}
		}
	}
